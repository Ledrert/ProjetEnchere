...
<body>
	...
	<form class="formSearch">
		<div id="divSearch">
			<label for="filtre">Filtres :</label> <input type="text"
				name="filtre"> <label for="categorie">Catégories :</label> <select
				name="categorie"><c:forEach var="libCat"
					items="${listeCat }">
					<option>${libCat }</option>
				</c:forEach></select>
			<button class="btnSearch" type="button" onclick="">Rechercher</button>
		</div>
		<div class="listEnchere">
			<%-- <c:forEach></c:forEach> --%>
			<!-- Pour chaque enchère : une case -->
			<div id="cardEnchere">
